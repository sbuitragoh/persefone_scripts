...
def single_image_inference(model, img, tokenizer, transform, device):
    ...
    with torch.no_grad():
        ...
        for i in range(128):
            ...
            output = model.vocab(
                model.transformer.decoder(model.query_pos(model.decoder(trg_tensor)), memory, tgt_mask=mask))
...
def run_epochs(model, criterion, optimizer, scheduler, train_loader, val_loader, epochs, tokenizer, target_path,
               device):
